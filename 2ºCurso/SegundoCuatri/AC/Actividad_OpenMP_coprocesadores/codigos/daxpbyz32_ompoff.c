...
#pragma omp target exit data map(delete: N, p, x[:N], y[0:N]) map(from: z[:N])
    ...
    printf("Tiempos: \n Target enter data(t2-t1) = %f \n Target temas ditribute parallel for(t3-t2) = %f \n target update(t4-t3) = %f \n Target teams distribute parallel for(t5-t4) = %f \n Target exit data(t6-t7) = %f", t2-t1, t3-t2, t4-t3, t5-t4, t6-t7);
