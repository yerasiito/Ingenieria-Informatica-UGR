...
public class Jugador implements Comparable<Jugador>{
    ...
    CasillaCalle hacerCalleSostenible(int ip){    
        CasillaCalle result = null;
        if(existeLaPropiedad(ip)){
            CasillaCalle propiedad = propiedades.get(ip);
            if(puedoConvertirCalle(propiedad)){
                result = propiedad.hacermeSostenible();
                if(result != null){
                    Diario.getInstance().ocurreEvento("El jugador " + nombre +
                            " hace sostenible la propiedad " + propiedad.getNombre());
                    propiedades.set(ip, result);
                }
            }    
        }
        return result;
