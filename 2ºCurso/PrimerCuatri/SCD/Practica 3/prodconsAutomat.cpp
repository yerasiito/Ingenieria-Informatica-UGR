...
// en orden arbitrario
// (version de productores y consumidores automatico)
// Numero de items fijo a 20, modificable

// Compilar con: mpicxx -std=c++11 -g -Wall -o prodconsAutomat prodconsAutomat.cpp
// Ejecutar con: mpirun -oversubscribe  -np 30 prodconsAutomat
...
int main( int argc, char *argv[] )
{
   ...
   if(num_procesos_actual % 2 == 0){
       num_consumidores = num_procesos_actual/2 -1; //La mitad de procesos -1 para consumidores(hay que reservar para buffer)
       num_productores = num_procesos_actual/2; //La otra mitad para productores
   }
   else{
       num_consumidores = num_procesos_actual/2; //La mitad de procesos -1 para consumidores(hay que reservar para buffer)
       num_productores = num_procesos_actual/2; //La otra mitad para productores
   }

   num_procesos_esperado = num_consumidores + num_productores + 1;
   tam_vector = num_procesos_esperado;
   num_items = num_consumidores * num_productores;

   if(id_propio == 0){ //Solo el primero
   cout << endl << "Se trabaja con: " << endl
        << num_consumidores << " consumidores" << endl
        << num_productores << " productores" << endl
        << num_items << " items" << endl << endl;
   }
