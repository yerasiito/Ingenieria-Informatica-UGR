...
//
// archivo: fumadores-fifo-semanticaSU.cpp
// Ejemplo de un monitor en C++11 con semántica SU, para el problema
// fumadores, con múltiples estanqueros y fumadores.
// Opcion LIFO (stack)
//
// -----------------------------------------------------------------------------
...
// *****************************************************************************
// clase para monitor buffer, version FIFO, semántica SU, un estanq. y un fum.
...
void ProdConsSU::escribir( int valor )
{
   // esperar bloqueado hasta que num_celdas_ocupadas < num_celdas_total
    if ( primera_ocupada == (primera_libre +1) % num_celdas_total )
        libres.wait();

   //cout << "escribir: ocup == " << num_celdas_ocupadas << ", total == " << num_celdas_total << endl ;
   assert( primera_ocupada != (primera_libre +1) % num_celdas_total );

   // hacer la operación de inserción, actualizando estado del monitor
   buffer[primera_ocupada] = valor ;
   primera_ocupada=(primera_ocupada+1)%num_celdas_total ;

   // señalar al fumador que hay un ingrediente en la mesa
   ocupadas.signal();
}
// *****************************************************************************
// funciones de hebras
...
{
   cout << "-------------------------------------------------------------------------------------" << endl
        << "Problema de fumadores (Varios estanqueros/fumadores, Monitor SU, buffer FIFO)." << endl
        << "-------------------------------------------------------------------------------------" << endl
        << flush ;
