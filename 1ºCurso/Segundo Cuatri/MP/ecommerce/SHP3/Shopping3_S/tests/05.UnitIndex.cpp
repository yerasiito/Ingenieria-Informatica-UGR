...
TEST_F(_01_Basics, Index_getIOnWhich) {
    ...
    key = 0;
    ies.build(es, key);
    SKIP_ASSERT_EQ_R(key, ies.getIOnWhich()) << "Building an index by Users reports 0" << endl;
    key = 1;
    ies.build(es, key);
    SKIP_ASSERT_EQ_R(key, ies.getIOnWhich()) << "Building an index by brand reports 1" << endl;
}
...
TEST_F(_03_Advanced, Index_U_BxU_bounds) {
    ...
    for (int u = 0; u < 1 /* U + 1 */; u++) {
        ...
        ASSERT_EQ_R(std::min((u + 1) * B, B * U), ies.upper_bound(user)) << lb << " The index by user built within experiment BxU upper bounds User u with (u+1)*B" << u << B << U << lb << ies.size() << endl;
    }
}
