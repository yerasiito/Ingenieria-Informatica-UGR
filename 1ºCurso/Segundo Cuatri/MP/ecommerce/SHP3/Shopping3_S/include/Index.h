...
public:
     ...
     *  - Value 0 indicates an indn ex over the users
     ...
     * @return a position in the Index
     */
    int upper_bound(const std::string & key);
...
int BusquedaBinaria (Pair pair[], std::string a_buscar, int util);
 ...
 * @param indx, Index from which to search the event
