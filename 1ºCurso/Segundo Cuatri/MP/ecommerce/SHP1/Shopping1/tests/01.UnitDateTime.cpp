...
TEST(_01_Basics, DateTime_Constructors) {
    DateTime c;
...
TEST(_01_Basics, DateTime_getters) {
...
TEST(_01_Basics, DateTime_set) {
    DateTime c;
    std::string date=TODAY;
    DateTime d;
    d.set(date);
    SKIP_ASSERT_STREQ_R(d.to_string().c_str(), date.c_str())<< "Setting an instance of DateTime with a valid string gives the same date" ;
}

TEST(_02_Intermediate, DateTime_isBefore) {
    DateTime defaultd;
    std::string date=TODAY;
    DateTime today;
...
TEST(_02_Intermediate, DateTime_weekDay) {
    std::string date=TODAY;
    DateTime d(date);
...
TEST(_03_Advanced, DateTime_BadValues) {
    DateTime d;

    d.set("2021-22-04 18:00:00");
    EXPECT_STREQ_R(d.to_string().c_str(),DATETIME_DEFAULT)<<"Setting a bad date or time gives the default datetime"<<endl;
    ...
    EXPECT_STREQ_R(d.to_string().c_str(),DATETIME_DEFAULT)<<"Setting a date with the incorrect format throws an exception"<<endl;
