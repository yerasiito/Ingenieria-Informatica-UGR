...
TEST(_01_Basics, Event_ConstructorBase) {
    Event e;
...
TEST(_01_Basics, Event_Setters_getters) {
    Event e;
...
TEST(_03_Advanced, Event_setType_Bad_Values) {
    Event e(ANYEVENT);
    std::string es=EVENT_DEFAULT;
    
    std::string s="CCCC";
    e.setType(s);
    EXPECT_STREQ_R("view", e.getType().c_str()) << "Setting the type of an event to a good value X gives X"<<endl;
    EXPECT_FALSE_R(e.isEmpty()) << "Setting the type of an event to a good value X gives X"<<endl;
    
    
}    
TEST(_03_Advanced, Event_Others_Bad_Values) {
    Event e(ANYEVENT);
    std::string es=EVENT_DEFAULT,s;
      
    s="";
    e.setProductID(s);
    EXPECT_STREQ_R(e.getProductID().c_str(),EMPTY_FIELD) << "Setting the productid of an event to a good value X gives X"<<endl;
    EXPECT_TRUE_R(e.isEmpty()) << "Setting the productid of an event to a good value X gives X"<<endl;
    
    double d=-3.1415;
    e.setPrice(d);
    EXPECT_EQ_R(e.getPrice(),-1) << "Setting the productid of an event to a good value X gives X"<<endl;
