      ...
      :precondition
        (and
            ...
            (exists (?te - tipoEdificio ?tu - tipoUnidad)
                (and
                    (esTipoEdificio ?e ?te) ; tipo de edificio en el que se recluta
                    (esTipoUnidad ?u ?tu) ; tipo de unidad que se recluta
                    (recluta ?tu ?te) ; en que tipo edificio se recluta la unidad
                    (en ?e ?l) ; El edificio esta en la localizacion
                )   
            )
            ...
            (or
                (and
                    (esTipoUnidad ?u Soldado) ; Si es un soldado
                    (Investigada spartan) ; debe investigar spartan
                )
                (not (EsTipoUnidad ?u Soldado)) ; O es cualquier otra unidad, no necesita investigar
            )
        )
    :effect
        (and
