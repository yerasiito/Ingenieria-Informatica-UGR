...
vector<string> algoritmosGeneticos {"AGG", "AGE", "SFHMA"};
vector<string> algoritmosMemeticos {"AM-All", "AM-Rand", "AM-Best", "SFHMA_hibrido"};
vector<string> operadores {"Aritmetrico", "BLX", "SPX"};
...
void showErrorUnknownAlgorithm(std::string_view algorithm) {
    std::cerr << "Algoritmo '" << algorithm << "' desconocido. Elige: 'sin parametro', SFMA, SFHMA_hibrido.\n";
}
...
int main(int argc, char **argv)
{
    ...
    if (!isAlgoritmoAPC && !isAlgoritmoGenetico && !isAlgoritmoMemetico) {
        showErrorUnknownAlgorithm(algoritmo);
        return EXIT_FAILURE;
    }

    if (isAlgoritmoAPC || isAlgoritmoMemetico) {
        if (argc <= 2)
    ...
    while (!nombreDatasets.empty())
    {
        ...
        for (int i = 0; i < ficheros; i++)
        {
            ...
            else if(algoritmo == "SFHMA")
                pesos = SFHMA(train, MAX_ITER, operador); // Dataset, tamPoblacion, porcentajeCruce
            else if(algoritmo == "SFHMA_hibrido")
                pesos = SFHMA_hibrido(train, MAX_ITER, operador); // Dataset, tamPoblacion, porcentajeCruce
