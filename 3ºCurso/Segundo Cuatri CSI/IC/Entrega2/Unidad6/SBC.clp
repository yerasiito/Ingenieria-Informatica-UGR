...
;;                    Diseniar un Sistema Basado en conocimiento que asesore a un cliente en una agencia de viaje                 ;;
;;                sobre qué destino elegir de forma que el sistema actúe tal y como lo hariais vosotros                           ;;
;;                                                                                                                                ;;
;;                                                        Hay 2 sistemas:                                                         ;;
;;      Sistema A: el usuario elige un destino de los ofertados directamente. Se le asigna transporte, alojamiento y actividad    ;;
;;      Sistema B: el usuario no sabe que viaje elegir. Se le pregunta sobre sus preferencias y se da una recomendacion           ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; SISTEMA BASADO EN CONOCIMIENTO DE RECOMENDACION DE VIAJES ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(deffacts Viajero
   ...
   (alojamiento aloj) ; alberge, hotel
)
...
=>
  (printout t "Sistema de Asesoramiento de Viajes. Hay una gran variedad a elegir así que" crlf)
  (printout t "se haran una serie de preguntas para poder recomendar la mejor eleccion." crlf crlf )
  (assert (modulo elegirModo))
  (assert (Aceptada no)) ;; Criterio de parada
)

(defrule imprimirDestinos
   (modulo elegirModo)
=>
  (printout t "Destinos ofertado en la agencia:" crlf)
  (printout t "##################################" crlf)
  (bind ?counter 0)
  (do-for-all-facts ((?v1 Viaje))
    (printout t "")
    (bind ?counter (+ ?counter 1))
    (printout t ?counter ": " ?v1:destino crlf)
  )
  (printout t "##################################" crlf)

  (assert (Maximo ?counter)) ; Guarda el maximo numero de destinos para comprobar luego la seleccion
)

;; Funcion para decidir si entrar en el sistema A o B:
;; - Si el usuario elige un destino concreto pasa al modulo A
;; - Si el usuario no sabe el destino (elige nose) pasa el modulo B (nuestro SE-viajes modularizado)
(defrule procesar-seleccion
   ?m <- (modulo elegirModo)
   (Maximo ?max)
=>
  (printout t crlf """Elige el numero de destino interesado o escribe 'nose' para
obtener una recomendacion basada en tus preferencias: """)

  (bind ?input (read))
  ; Comprueba que sea un numero o la cadena 'nose', sigue preguntando si es incorrecto
  (while (or (not (or (integerp ?input) (eq ?input nose))) (and (integerp ?input) (or (> ?input ?max) (< ?input 1) )))
      (printout t """Selección inválida. 
Por favor, selecciona un numero en el rango 1-""" ?max """ o la opcion 'nose':""")
      (bind ?input (read))
  )

  ; Entra en el modulo B
  (if (eq ?input nose)
   then
      (assert (modulo B))
  )

  ; Entra en el modulo A
  (if (integerp ?input)
  then
   (bind ?counter 1)
   (do-for-all-facts ((?v1 Viaje))
...
(deffacts listaAlojamientos
  (Alojamiento (codigo ALOJ1) (nombre "Hotel 5 estrellas 'Las maravillas'"))
  (Alojamiento (codigo ALOJ2) (nombre "Albergue 'Sin Chapuzas'"))
  (Alojamiento (codigo ALOJ3) (nombre "Cabanias 'El Refugio'"))
  (Alojamiento (codigo ALOJ4) (nombre "Hotel Boutique 'La Perla'"))
  (Alojamiento (codigo ALOJ5) (nombre "Resort 'Paraiso Azul'"))
  (Alojamiento (codigo ALOJ6) (nombre "Hostal 'El Descanso'"))
  (Alojamiento (codigo ALOJ7) (nombre "Villa 'Bella Vista'"))
  (Alojamiento (codigo ALOJ8) (nombre "Posada 'La Tranquilidad'"))
  (Alojamiento (codigo ALOJ9) (nombre "Apartamento 'Sol y Mar'"))
  (Alojamiento (codigo ALOJ10) (nombre "Camping 'Aventura al Aire'"))
)


(deffacts listaActividades
  (Actividad (codigo SPA1) (nombre "SPA 'Masajes de otra dimensión'"))
  (Actividad (codigo ESC) (nombre "Escalada 'Peñón La Sierra'"))
  (Actividad (codigo SURF) (nombre "Surf 'Olas Salvajes'"))
  (Actividad (codigo PNTB) (nombre "Paintball 'La Batalla'"))
  (Actividad (codigo KAYK) (nombre "Kayak 'Ruta Marina'"))
  (Actividad (codigo BICI) (nombre "Bicicleta 'Rutas Verdes'"))
  (Actividad (codigo PESCA) (nombre "Pesca 'Mar Adentro'"))
  (Actividad (codigo GOLF) (nombre "Golf 'Campo Esmeralda'"))
  (Actividad (codigo AVIS) (nombre "Avistamiento de aves 'El Paraíso'"))
  (Actividad (codigo SPRT) (nombre "Deportes extremos 'Adrenalina Total'"))
)


(defrule inicioSistemaA
   (modulo A)
   (Adecuado (codigo ?cod))
   (Viaje (codigo ?cod) (edad ?edad) (numero ?compania) (temporada ?temporada) (duracion ?duracion) (lugar ?lugardestino)
          (ubicacion ?ubicacion) (tipo ?tipodestino) (precio ?presupuesto))
   =>
   
   ;; Logica por defecto
   
   ;; Asumiremos que le va a gustar nuestro viaje montado actual
   (assert (Viajero edad ?edad)) ; joven, media, mayor
   (assert (Viajero compania ?compania)) ; una, varias
   (assert (Viajero temporada ?temporada)) ; invierno, primavera, verano, otonio, cualquiera
   (assert (Viajero duracion_de_viaje ?duracion)) ; corto, medio, largo
   (assert (Viajero presupuesto ?presupuesto)) ; numero
   (assert (Viajero lugar_de_destino ?lugardestino)) ; rural, urbano, intermedio
   (assert( Viajero ubicacion_geografica ?ubicacion)) ; nacional, europeo, internacional
   (assert (Viajero tipo_destino ?tipodestino)) ; Playa, montania, interior

   ; Le damos un transporte, alojamiento y actividad por defecto las mas caras
   (assert (Viajero transporte avion))
   (assert (Viajero alojamiento ALOJ1))
   (assert (Viajero actividad SPA1))
)

;; Muestra el viaje elegido
(defrule mostrarViaje
   (modulo A)
   (Viajero edad ?edad)
   (Adecuado (codigo ?cod))
   (Viaje (codigo ?cod) (destino ?dest) (precio ?precio) (numero ?numero) (temporada ?temporada)
                (duracion ?duracion) (lugar ?lugar) (ubicacion ?ubicacion)
                (tipo ?tipo))
=>
  (printout t crlf "###########################################" crlf)
  (printout t "Has seleccionado el destino " ?dest crlf)
  (printout t "El precio del viaje es " ?precio "€." crlf)
  (printout t "Este viaje va dirigido a personas de edad " ?edad "." crlf)
  (printout t "Perfecto para " ?numero " persona/s." crlf)
  (printout t "El destino está disponible durante la temporada de " ?temporada "." crlf)
  (printout t "El viaje tiene una duración " ?duracion "." crlf)
  (printout t "Se sitúa en un ambiente " ?lugar "." crlf)
  (printout t "El destino es " ?ubicacion "." crlf)
  (printout t "Hay " ?tipo " impresionantes para disfrutar." crlf)
  (printout t "###########################################" crlf crlf)

   ; Pregunta si acepta o no
   (assert (modulo Ofrecer))
)

(defrule mostrarPreferencias
   (modulo A)
   (modulo Ofrecer)

   (not(OfertaRecibida si))

   ?acept <- (Aceptada no)
   
   (Viajero alojamiento ?aloj)
   (Alojamiento (codigo ?aloj) (nombre ?nombreAloj))

   (Viajero transporte ?trans)

   (Viajero actividad ?act)
   (Actividad (codigo ?act) (nombre ?nombreAct))

   (Adecuado (codigo ?cod))
   (Viaje (codigo ?cod) (destino ?dest))
=>
  (printout t crlf "Hemos seleccionado para ti: " crlf)
  (printout t "Alojamiento: " ?nombreAloj crlf)
  (printout t "Transporte: " ?trans crlf)
  (printout t "Actividad: " ?nombreAct crlf)
  
  (printout t crlf "¿Aceptas el viaje a " ?dest "?" crlf)
   (bind ?resp (read))
...
(defrule MotivoRechazoA
   ...
   ?adec <- (Adecuado (codigo ?cod))

=>
   (printout t "¿Cual es tu motivo de rechazo? Elige una" crlf)
   (printout t "(transporte|actividad|alojamiento|otro): " crlf)
   (bind ?motivo (read))
   (while (and(not (eq ?motivo transporte)) (not (eq ?motivo actividad)) (not (eq ?motivo alojamiento)) (not (eq ?motivo otro)))
   ...
   ; Si tiene otro motivo, no le gusta el viaje. Preguntamos de nuevo por otro
   (if (eq ?motivo otro)
   then
      (retract ?m)
      (printout t "Lo sentimos, pero no tenemos disponible el viaje que desea." crlf)
      (printout t "¿Deseas elegir otro o salir del sistema?(si|no)")
      (bind ?resp (read))
      (while (and(not (eq ?resp si)) (not (eq ?resp no)))
   ...
   ; Intentamos cambiar el motivo
   (if (not(eq ?motivo otro))
   then
      (assert (MotivoRechazo ?motivo)) ; Hecho con el motivo de rechazo: temporada, transporte, etc.
   )
)
...
(defrule preguntarActividad
   (modulo A)
   ?m <- (modulo Preguntar)
   (not (Viajero actividad ?))
=>
   (printout t crlf "¿Cuales de las siguientes actividades quieres?: " crlf)
   
   ;; Imprime todas las actividades
   (bind ?counter 0)
   (do-for-all-facts ((?act Actividad))
      (printout t "")
      (bind ?counter (+ ?counter 1))
      (printout t ?counter ": " ?act:nombre crlf)
   )

   ;; Pregunta al usuario
  (bind ?input (read))
  ; Comprueba que sea un numero o la cadena 'nose', sigue preguntando si es incorrecto
  (while (or (not (or (integerp ?input) (eq ?input nose))) (and (integerp ?input) (or (> ?input ?counter) (< ?input 1) )))
      (printout t """Selección inválida. 
Por favor, selecciona un numero en el rango 1-""" ?counter """ o la opcion 'nose':""")
      (bind ?input (read))
  )

  ; Segun la respuesta hace assert de uno u otro
  
  ; Asigna la actividad por defecto 
  (if (eq ?input nose)
   then
      (assert (Viajero actividad SPA1))
  )

  ; Asigna la actividad elegida
  (if (integerp ?input)
  then
   (bind ?counter 1)
   (do-for-all-facts ((?act Actividad))
     ...
     (if (eq ?counter ?input)
      then
        (assert (Viajero actividad ?act:codigo))
        (retract ?m)
        (assert (modulo Ofrecer))
     )
     (bind ?counter (+ ?counter 1))
...
;; Pregunta el alojamiento
(defrule preguntarAlojamiento
   (modulo A)
   ?m <- (modulo Preguntar)
   (not (Viajero alojamiento ?))
=>
   (printout t crlf "¿Cual de los siguientes alojamientos quieres?: " crlf)
   
   ;; Imprime todas las actividades
   (bind ?counter 0)
   (do-for-all-facts ((?act Alojamiento))
      (printout t "")
      (bind ?counter (+ ?counter 1))
      (printout t ?counter ": " ?act:nombre crlf)
   )
...
Por favor, selecciona un numero en el rango 1-""" ?counter """ o la opcion 'nose':""")
  ...
  then
   ...
   (do-for-all-facts ((?act Alojamiento))
     ...
     (if (eq ?counter ?input)
      then
        (assert (Viajero alojamiento ?act:codigo))
        (retract ?m)
        (assert (modulo Ofrecer))
     )
     (bind ?counter (+ ?counter 1))
...
;; ¿Transporte? ;;
(defrule pregunta_transporteA
   (modulo A)
   ?m <- (modulo Preguntar)
   (not (Viajero transporte ?))
=>
   (printout t "Que medio de transporte quieres? (autobus|tren|avion|barco): ")
   (bind ?resp (read))
   (while (and (not (eq ?resp autobus)) (not (eq ?resp tren)) (not (eq ?resp avion)) (not (eq ?resp barco)))
      (printout t "Valor Incorrecto. Introduce un valor (autobus|tren|avion|barco): ")
      (bind ?resp (read))
   )
...
(defrule inicioSistemaB
   (modulo B)
=>
   (printout t crlf "A continuacion se te haran algunas preguntas sobre ti para elegir el viaje mas adecuado." crlf)
   (assert (modulo Preguntar))
)
