...
public class AgenteLRTAStar extends AbstractPlayer{
	...
	private ACTIONS AlgoritmoLRTA(StateObservation mundo){
		...
		nodoLocal = new Nodo(posicionJugador, ACTIONS.ACTION_NIL, null, portalFin, true);		
		...
		nodosExpandidos++;
		numeroAcciones++;
		return minimo.getAccion();
	}
	//FIN DEL ALGORITMO
	
	void imprimeInfo(long tInicio) {
		System.out.println("Tiempo de busqueda total: " + (tiempoTotalms/1000000) + " ms");
	...
	public ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {	
		...
		ACTIONS accion = AlgoritmoLRTA(stateObs); 		// Calcular accion a ejecutar
		long tFin = System.nanoTime();
		tiempoTotalms += (tFin - tInicio);

		if(nodoLocal.isEstadoFinal()) {
			imprimeInfo(nodosExpandidos);
		}
